...
router.get("/products", async (req, res, next) => {
  try {
    ...
    console.log(
      `Retrieved ${products.length} products (page ${pageNum}/${totalPages})`
    );
...
router.post("/products", async (req, res, next) => {
  try {
    ...
    const existingProduct = await ProductQueries.getProductByName(
      req.body.name
    );
    if (existingProduct) {
...
router.put("/products/:id", async (req, res, next) => {
  try {
    ...
    console.log(
      "PUT /api/inventory/products/:id called with ID:",
      productId,
      "and data:",
      req.body
    );
...
router.delete("/products/:id", async (req, res, next) => {
  try {
    ...
    console.log(
      "DELETE /api/inventory/products/:id called with ID:",
      productId
    );
...
router.post("/recipe-links", async (req, res, next) => {
  try {
    ...
    const { RecipeIngredientProductQueries } = await import(
      "../utils/inventory-database.js"
    );

    // Create link with full validation
    const result =
      await RecipeIngredientProductQueries.createLinkWithValidation(req.body);
...
router.get("/recipe-links/:recipeId", async (req, res, next) => {
  try {
    ...
    console.log(
      "GET /api/inventory/recipe-links/:recipeId called with ID:",
      recipeId
    );
    ...
    const links = await RecipeIngredientProductQueries.getRecipeProductLinks(
      recipeId
    );

    console.log(
      `Retrieved ${links.length} recipe ingredient links for recipe ${recipeId}`
    );
...
router.delete("/recipe-links/:id", async (req, res, next) => {
  try {
    ...
    console.log(
      "DELETE /api/inventory/recipe-links/:id called with ID:",
      linkId
    );
...
router.post("/products/:id/restock", async (req, res, next) => {
  try {
    ...
    console.log(
      "POST /api/inventory/products/:id/restock called with ID:",
      productId,
      "and data:",
      req.body
    );
    ...
    const { InventoryTransactionQueries } = await import(
      "../utils/inventory-database.js"
    );

    // Validate product exists
    const product =
      await InventoryTransactionQueries.validateProductForQuantityUpdate(
        productId
      );
    if (!product) {
      return res.status(404).json({
    ...
    const result = await InventoryTransactionQueries.restockProduct(
      productId,
      parseFloat(quantity),
      {
...
router.post("/products/:id/adjust", async (req, res, next) => {
  try {
    ...
    console.log(
      "POST /api/inventory/products/:id/adjust called with ID:",
      productId,
      "and data:",
      req.body
    );
    ...
    const validation = await InventoryTransactionQueries.validateQuantityChange(
      productId,
      quantityChangeNum
    );
    if (!validation.isValid) {
    ...
    const result = await InventoryTransactionQueries.adjustProductQuantity(
      productId,
      quantityChangeNum,
      {
        reference_type: "manual",
    ...
    console.log(
      "Product quantity adjusted successfully:",
      productId,
      "change:",
      quantityChangeNum
    );
...
router.get("/transactions", async (req, res, next) => {
  try {
    ...
    console.log(
      "GET /api/inventory/transactions called with query:",
      req.query
    );
    ...
    if (
      product_id &&
      (isNaN(parseInt(product_id)) || parseInt(product_id) < 1)
    ) {
      return res.status(400).json({
    ...
    console.log(
      `Retrieved ${transactions.length} transactions (page ${pageNum}/${totalPages})`
    );
...
router.get("/dashboard", async (req, res, next) => {
  try {
    ...
    console.log(
      `Dashboard summary generated: ${summary.total_products} products, ${summary.low_stock_count} low stock, ${summary.out_of_stock_count} out of stock`
    );
